...
public class ServiceRequestMailSenderService {

  private final JavaMailSender mailSender;

  public void sendEmailConfirmation(String clientEmail, ServiceRequest serviceRequest) {
    ...
    try {
      ...
      try (InputStream inputStream =
          getClass().getResourceAsStream("/templates/emailConfirmationServiceRequest.html")) {
        htmlEmailTemplate = new String(inputStream.readAllBytes(), StandardCharsets.UTF_8);
      } catch (IOException e) {
        throw new RuntimeException(e);
      }

      if (serviceRequest.getDealershipLocation().name().equals("BRASOV")) {
        mapLink =
      ...
      } else if (serviceRequest.getDealershipLocation().name().equals("BUCHAREST")) {
        mapLink =
      ...
      finalHtmlEmailTemplate =
          htmlEmailTemplate
              .replace("${firstName}", serviceRequest.getFirstName())
              .replace("${lastName}", serviceRequest.getLastName())
              .replace("${formattedDateTime}", serviceRequest.getDateTime().toString())
              .replace("${modelType}", serviceRequest.getModelType().getDescription())
              .replace("${location}", serviceRequest.getDealershipLocation().getDescription())
